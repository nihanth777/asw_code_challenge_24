...
uint8_t update_address(uint8_t* a, uint8_t* b, uint8_t* c, uint8_t* d, uint8_t* e, uint8_t* f, uint8_t* g, uint8_t* h) {
...
float twosCompToDec(uint8_t two_compliment_val)
{
...
uint8_t get_full_configuration(){
    uint8_t buffer;
    if (i2c_read((uint8_t)DEVICE_ADDRESS, (uint8_t)CTRL_REG2, sizeof(uint8_t), &buffer) == STATUS_OK) {
        return buffer;
    } else {
        // Handle error
        return 0;
...
uint8_t get_output_data_rate(){
    ...
    if (i2c_read((uint8_t)DEVICE_ADDRESS, (uint8_t)CTRL_REG2, sizeof(uint8_t), &buffer) == STATUS_OK) {
        ...
        do0 = (uint8_t)get_nth_bit_value(buffer, 2);
        do1 = (uint8_t)get_nth_bit_value(buffer, 3);
        do2 = (uint8_t)get_nth_bit_value(buffer, 4);
    ...
    return (uint8_t)-1.0; // Default value indicating no match found
    } else {
        // Handle error
        return 0;
    }
...
uint8_t set_output_data_rate(uint8_t ST, uint8_t FAST_ODR, uint8_t DO0, uint8_t DO1, uint8_t DO2, uint8_t OM0, uint8_t OM1, uint8_t TEMP_EN){  // All parameters are given as input as final usage wasnt clear so given maximum control here
    ...
    uint8_t buffer;
    buffer = update_address(&ST, &FAST_ODR, &DO0, &DO1, &DO2, &OM0, &OM1, &TEMP_EN);

    
    if (i2c_write((uint8_t)DEVICE_ADDRESS, (uint8_t)CTRL_REG2, sizeof(uint8_t), &buffer) != STATUS_OK) {
       return (uint8_t)1;
    }
}
//-----------------------------------------------------------------------------------
...
uint8_t ctrl_interrupt_pin(uint8_t pin){ //pin = 0||1, 0-disable, 1-enable
    ...
    if (pin) {
        buffer |= 0x01;  // Set LSB to 1 to enable interrupt pin
    } else {
        buffer &= 0xFE;  // Clear LSB to 0 to disable interrupt pin
    }
    if (i2c_write((uint8_t)DEVICE_ADDRESS, (uint8_t)CTRL_REG2, sizeof(uint8_t), &buffer) != STATUS_OK) {
        return (uint8_t)1;
    }
...
float axis_output_data(uint8_t axis){
    uint8_t reg_L, reg_H;
    switch(axis) {
        case 'X':
            reg_L = OUT_X_L;
    ...
    uint8_t buffer;
    float value_L, value_H;

    if (i2c_read((uint8_t)DEVICE_ADDRESS, (uint8_t)reg_L, sizeof(uint8_t), &buffer) == STATUS_OK) {
       value_L =  twosCompToDec(buffer);
    } else {
        // Handle error
        value_L = 0.00;
    }
    if (i2c_read((uint8_t)DEVICE_ADDRESS, (uint8_t)reg_H, sizeof(uint8_t), &buffer) == STATUS_OK) {
        value_H = twosCompToDec(buffer);
    } else {
        // Handle error
        value_H = 0.00;
    }
